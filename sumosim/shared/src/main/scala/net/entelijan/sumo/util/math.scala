...
/** Point in a two dimensional space
  */
case class Point2(val xpos: Double, val ypos: Double) {
  ...
  override def hashCode: Int = xpos.hashCode + ypos.hashCode
}
  ...
  def direction: Double = {
    if (x > 0) normalize(atan(y / x))
      ...
      throw new IllegalArgumentException(
        "direction not defined for (%.2f %.2f)" format (x, y)
      )
  }

  /** Rotates the angle for a certain angle in rad Returns a new (immutable)
    * vector
    */
  def rot(rad: Double): Vec2 = {
    val a = x * cos(rad) - y * sin(rad)
    val b = y * cos(rad) + x * sin(rad)
    new Vec2(a, b)
  }

  /** Multiplication of a scalar with the vector Returns a new (immutable)
    * vector
    */
  def *(v: Double) = new Vec2(v * x, v * y)

  /** Addition of a scalar to the vector Returns a new immutable vector
    */
  def +(v: Vec2) = new Vec2(v.x + x, v.y + y)

  /** Returns the enclosed angle of two vectors The angle starts always from the
    * original vector It is always smaller than pi
    */
...
trait TrigUtil {
  ...
  /** Brings an angle represented in rad in the intervals [0, pi] or [0, -pi]
    */
  def normalize(rad: Double): Double = {
    val mod = rad % (2 * Pi)
    if (mod > Pi) mod - 2 * Pi
  ...
  /** Converts a deg represented angle to rad scale
    */
  def toRad(deg: Double): Double = Pi / 180 * deg

  /** Converts a rad represented angle to deg scale
    */
  def toDeg(rad: Double): Double = 180 / Pi * rad
